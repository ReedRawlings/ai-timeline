...
1. ~~BTOS biweekly series~~ **Done 2026-07-07** via a browser session on the (JS-gated) download portal: full national series ingested for both wording regimes — 54 cycles old wording (202319–202520, 3.7%→10.0%) + 16 cycles new wording (202524–202613, 17.3%→20.6%), current + intended use, each row carrying its reference period. Sources: `National.xlsx` (new series) and `AI Core Questions.xlsx` (old series, Historical tab). Fielded instruments captured verbatim: core V4 Cycles 3–4 (AI items at Q7/Q24) and "BTOS Core and AI Content" V4 Cycle 2 (full AI supplement, Q23–Q35). Shutdown gap: cycles 202521–202523 (Oct 6–Nov 16, 2025) were never collected — structural, per Census's data dictionary. Remaining BTOS nice-to-haves: per-cycle standard errors (published in the same files, not ingested); supplement function-level table (`AI_Supplement_Table_2026.xlsx`, out of scope per §8.2); ~~disambiguating w35141's worker-AI headline~~ (resolved 2026-07-07 from the supplement table: 23%/41% = Q30, broader worker-AI, 22.6% firm-weighted; Q31 genAI-specific = 20.8% — both now estimate rows). **BTOS also has a documented no-auth JSON API** (census.gov/hfp/btos/api_docs — periods/questions/data endpoints; core questions only, supplement not loaded as of 2026-07) — likely the cleanest sweep mechanism once verified against the XLSX values.
2. ~~Gallup verbatim stem~~ **Done 2026-07-07**: the image-locked Q4 topline PDF, rendered in a browser, carries the full items — Q619A "How often do you use artificial intelligence in your role?" (7-option scale) and Q653A (full org-integration stem). All four Gallup question rows upgraded to `published`; per-wave distributions ingested (`gallup-wf-q619a-*`); org item Q3/Q4 2025 values added. Remaining Gallup nice-to-haves: Q1 2026 topline PDF (for its distribution), pre-break org values, and the item-n vs survey-n subsample question (documented in the question rows).
3. ~~RPS post-2024 wave field dates~~ **Resolved 2026-07-07** via the authors' Nov 2025 Fed post: quarterly since Aug 2024 (pilot Jun 2024). **Bigger find: RPS revised its own history in Oct 2025** — a question-sequencing test showed earlier waves underestimated usage, and 2024/Feb/May-2025 waves were reweighted (Aug 2024 overall 39.4% → 44.6%). Pre-revision rows retained and flagged; revised-series rows added (`rps-*-rev`, Aug 2025 wave). Remaining: exact wave label behind the tracker's May 2026 headline values.
...
5. SHED: **≥2-waves check resolved NEGATIVE 2026-07-07** — the 2024 questionnaire appendix (fetched) contains no AI item; Oct 2025 was SHED's first AI wave. Stays a candidate; recheck when the Oct 2026 wave publishes (~May 2027).
