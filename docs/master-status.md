...
- **Compute & revealed-usage indicators** — **State: Parked** → parked.md (added 2026-07-07). Telemetry counterpart to the adoption panel (Epoch AI database + OpenRouter rankings verified; others pending); separate construct, never blended with survey estimates.
- **Adoption/usage panel** (`data/adoption/`) — **State: Development** (Parked → Development 2026-07-07 under Reed's mid-session delegation "act without me, keep a record"; PRD provisionally approved, §8 resolved by Claude — **all recorded decisions await Reed's reflection pass**). v1 seed: `questions.csv` (21 question versions, wording breaks as linked versions) + `estimates.csv` (58 publisher-primary estimates) across Pew ATP + workers series, Gallup Workforce Study, Census BTOS, RPS/GenAI Adoption Tracker; `scripts/validate-adoption.py` in CI; `data/adoption` added to the Vercel deploy-skip. Detail: `data/adoption/README.md`, `docs/prds/adoption-usage-panel-prd.md` (+ `-recon.md` source record).
  **Maintenance (once Deployed): Quarterly** (`find-adoption-waves-quarterly` skill, created 2026-07-07; sweep spec in PRD §9).
  **Open:** Reed's review of the delegated decisions (PRD §8 resolutions block); backfill queue — ~~BTOS biweekly points~~ (done 2026-07-07: 140 rows, both wording regimes, via browser session) and ~~Gallup stem~~ (found 2026-07-07 in the image-locked Q4 topline: Q619A, 7-option scale; all Gallup rows now `published`) resolved; remaining: RPS post-2024 wave dates, SHED promotion on the ≥2-waves check, BTOS supplement Q30/Q31 disambiguation, BTOS API tab (spotted 2026-07-07, unexplored).
