...
1. **Curated sources are the discovery engine — and a reported gate (conventions §6):** run the `site:` searches on the Substacks, the Superintelligence **month archive** (`/topics` — not just the homepage, which only shows recent days), the **lab research feeds** (OpenAI/Anthropic/DeepMind — they headline their own novel work), and Reddit for social/cultural moments, **before** any broad query — and list in the review summary which curated sources you actually hit *for the target period*. Skipping them — or reaching only a source's stale homepage and calling it checked — is the single most repeated failure of this skill (2026-07 passes missed the Pope Leo XIV encyclical, and missed OpenAI's May 20 Erdős-conjecture disproof that Superintelligence had actually covered). Per §6's access caveat, when curated sources come up empty, widen to first-tier outlets; do not skip verification.
